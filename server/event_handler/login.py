...
def run(sc, parameters):
    ...
    sql="SELECT id,username from users where username='%s' and password='%s'"%(parameters[0], md5(parameters[1]))
    print(sql)
    #r = c.execute("SELECT id,username from users where username='%s' and password='%s'", (parameters[0], md5(parameters[1])))
    c.execute(sql)
    rows = c.fetchall()
