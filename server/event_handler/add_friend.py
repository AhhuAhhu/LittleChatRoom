...
def run(sc, parameters):
    ...
    c = database.get_cursor()
    sql="SELECT 1 from friends where from_user_id=%d and to_user_id=%d"%(user_id,uid)
    c.execute(sql)
    r=c.fetchall()
#    r = c.execute('SELECT 1 from friends where from_user_id=? and to_user_id=?', [user_id, uid]).fetchall()
    ...
    c = database.get_cursor()
    sql="insert into friends (from_user_id,to_user_id,accepted) values (%d,%d,0)"%(user_id,uid)
    c.execute(sql)
    r=c.fetchall()
